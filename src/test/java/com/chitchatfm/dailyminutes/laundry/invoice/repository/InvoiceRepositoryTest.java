...
@EnableJdbcRepositories(basePackages = "com.dailyminutes.laundryapp.invoice.repository")
@ComponentScan(basePackages = "com.dailyminutes.laundryapp.invoice.domain")
class InvoiceRepositoryTest {
    ...
    void testSaveAndFindInvoiceWithItems() {
        ...
        InvoiceEntity invoice = new InvoiceEntity(null, "SWIPE123", "9876543210", LocalDateTime.now(), new BigDecimal("50.00"), new BigDecimal("5.00"), new BigDecimal("2.00"));
        ...
        InvoiceItemEntity item1 = new InvoiceItemEntity(null, savedInvoice.getId(), 201L, 1, new BigDecimal("25.00"), new BigDecimal("2.50"));
        InvoiceItemEntity item2 = new InvoiceItemEntity(null, savedInvoice.getId(), 202L, 2, new BigDecimal("10.00"), new BigDecimal("1.00"));
        ...
        assertThat(foundItems.stream().map(InvoiceItemEntity::getCatalogId)).containsExactlyInAnyOrder(201L, 202L);
    }
    ...
    void testFindBySwipeInvoiceId() {
        invoiceRepository.save(new InvoiceEntity(null, "SWIPE456", "1112223333", LocalDateTime.now(), new BigDecimal("100.00"), new BigDecimal("10.00"), new BigDecimal("5.00")));
        Optional<InvoiceEntity> foundInvoice = invoiceRepository.findBySwipeInvoiceId("SWIPE456");
        assertThat(foundInvoice).isPresent();
        assertThat(foundInvoice.get().getCustomerPhoneNumber()).isEqualTo("1112223333");
    }
    ...
    void testUpdateInvoice() {
        InvoiceEntity invoice = new InvoiceEntity(null, "UPDATE789", "5556667777", LocalDateTime.now(), new BigDecimal("70.00"), new BigDecimal("7.00"), new BigDecimal("3.00"));
        InvoiceEntity savedInvoice = invoiceRepository.save(invoice);
    ...
    void testDeleteInvoiceAndAssociatedItems() {
        ...
        InvoiceEntity invoice = new InvoiceEntity(null, "DELETE000", "0001112222", LocalDateTime.now(), new BigDecimal("20.00"), new BigDecimal("2.00"), new BigDecimal("1.00"));
        InvoiceEntity savedInvoice = invoiceRepository.save(invoice);

        // Create and save InvoiceItem
        InvoiceItemEntity item1 = new InvoiceItemEntity(null, savedInvoice.getId(), 301L, 1, new BigDecimal("20.00"), new BigDecimal("2.00"));
        invoiceItemRepository.save(item1);
